...
func main() {
	...
	mux.HandleFunc("/test-hubspot", testHubspotHandler)
    ...
    fmt.Println("  GET /test-hubspot?email=<email> - Test HubSpot contact lookup")
...
func testHubspotHandler(w http.ResponseWriter, r *http.Request) {
    ...
    exists, err := client.IsExistingContact(email)
    if err != nil {
        http.Error(w, fmt.Sprintf("Error checking contact: %v", err), http.StatusInternalServerError)
        return
    }
